...
public class SetMinimalityChecker {
	 ...
	 * @param subsetChecker A {@link SubsetChecker} for type {@code S}
	...
	public static <S extends SetRepresent<?> & Comparable<S>> boolean isMinimal(NavigableSet<S> col, Set<?> test,
			MatchProvider<S, Set<?>> matchProvider, SubsetChecker<S> subsetChecker) {
		...
		while (subsetIter.hasNext()) {
			if (subsetChecker.subsetOf(subsetIter.next(), test)) {
